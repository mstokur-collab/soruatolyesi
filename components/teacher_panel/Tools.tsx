...
import LocalCurriculumManager from './LocalCurriculumManager';

export const Tools: React.FC = () => {
    const { userType, isAdmin, isDevUser } = useAuth();
    const {
    ...
    if (userType === 'guest') {
        return (
            <div className="p-6 text-center max-w-sm mx-auto">
                <h3 className="text-xl font-bold text-yellow-300">Bu özelliği kullanmak için giriş yapın</h3>
                <p className="mt-2 text-slate-400">Giriş yaparak AI araçlarını kullanabilir ve verilerinizi yönetebilirsiniz.</p>
            </div>
        );
    }

    const canEditGlobalCurriculum = isAdmin || isDevUser;
    ...
    return (
        <div className="w-full max-w-6xl mx-auto p-4 sm:p-6 space-y-6">
            ...
            <LocalCurriculumManager />

            {canEditGlobalCurriculum && (
                <div className="rounded-xl border border-violet-500/30 bg-slate-900/40 p-4">
                    <CurriculumManager />
                </div>
            )}
            ...
            <Modal
                ...
                title="Çözdüğüm Soruları Sıfırla"
